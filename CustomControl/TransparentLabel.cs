	...
	public class TransparentLabel : Label
	{
		...
		private void DrawParentWithBackControl(PaintEventArgs pevent)
		{
			...
			for (int num = base.Parent.Controls.Count - 1; num >= 0; num--)
			{
				...
				if (base.Bounds.IntersectsWith(control.Bounds))
				{
		...
		{
			using (Bitmap image = new Bitmap(c.Width, c.Height, PixelFormat.Format32bppArgb))
			{
				...
				pevent.Graphics.DrawImage(image, base.ClientRectangle, new Rectangle(num, num2, base.ClientRectangle.Width, base.ClientRectangle.Height), GraphicsUnit.Pixel);
			}
		}
